...
def main():
    ...
    if Novelty_detection is True:
    	ustc_names = os.listdir(root_src_ustc)

    	for name in ustc_names:
    		known_catgs.append(name)
    		novel_catgs.append(name)
    		shutil.copytree(os.path.join(root_src_ustc,name),os.path.join(root_dst_val,name))
    		shutil.copytree(os.path.join(root_src_ustc,name),os.path.join(root_dst_test,name))
...
if __name__ == '__main__':
    ...
    for i in range(times):
        ...
        # with open('../record/novelty_5_5.txt','a+') as f:
        #     tmp = []

        #     tmp.append(known)

        #     tmp.extend(novelty.tolist())
